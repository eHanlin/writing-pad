
export const CLOSE = 'hide';
export const START_DRAWING = 'startDrawing';
export const BOARD_START_DRAWING = 'board:startDrawing';
export const STOP_DRAWING = 'stopDrawing';
export const BOARD_STOP_DRAWING = 'board:stopDrawing';
export const DRAWING = 'drawing';
export const BOARD_DRAWING = 'board:drawing';
export const UPDATE_HISTORY = 'updateHistory';
export const SWITCH_HISTORY_KEY = 'switchHistoryKey';
export const CHANGE_PEN_COLOR = 'changePenColor';
export const CHANGE_PEN_SIZE = 'changePenSize';
export const CHANGE_DRAWING_COLOR = 'changeDrawingColor';
export const CHANGE_DRAWING_SIZE = 'changeDrawingSize';
export const RESET_COLOR_PICKER = 'resetColorPicker';
export const OPEN_COLOR_PICKER = 'openColorPicker';
export const CLOSE_COLOR_PICKER = 'closeColorPicker';
export const CLEAR = 'clear';

